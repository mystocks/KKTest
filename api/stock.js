...
function requestOptionals() {
    ...
    var promise = Service.request({
        ...
        url: `${Service.BaseOptionalUrl}MyZXG`,
    }).then(function (res) {
        if (res.statusCode == 200) {
            if (res.data.result.code == 0) {
...
function getQuotation({id} = {}) {
    ...
    var promise = Service.request({
        ...
        url: `${Service.BaseQuotaUrl}OneQuotation`,
    }).then(function (res) {
      console.log('stock quotation raw : ', res)
        if (res.statusCode == 200) {
...
function getRelatives({id, classType = 0, order = false} = {}) {
    ...
    console.log("Enter getRelatives...");
...
function getCustomDetail(aryGoodsId = []) {
    if (!aryGoodsId)
        return
    console.log("getCustomDetail_1:", aryGoodsId, "length:", aryGoodsId.length);
    var reqFileds = []
    reqFileds.push(GoodsParams.GOODS_NAME);   // 股票名称
    reqFileds.push(GoodsParams.ZXJ);          // 最新价
    reqFileds.push(GoodsParams.ZDF);          // 涨跌幅
    ...
    var promise = Service.request({
        ...
        url: `${Service.BaseQuotaUrl}RealNewPrice`,
    }).then(function (res) {
        if (res.statusCode == 200) {
            //console.log('custom detail raw : ', res.data)
            var result = parser.parseCustomDetail(res.data)
            //console.log('custom detail raw22 : ', result)
            return result
        } else {
            return Promise.reject(StaticStrings.kGetDataErrorInfo)
        }
